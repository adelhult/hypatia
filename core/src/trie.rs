...
use std::hash::Hash;
use std::fmt::Debug;
use std::iter::{Chain, Flatten, Map, Once};
use std::path::Iter;
...
#[derive(Debug, Clone, PartialEq)]
struct StringTrie<V> (Trie<char, V>)
    where
        V: Clone + PartialEq;
        ...
        V: Clone + PartialEq
{
    ...
    fn entries(&self) -> Box<dyn Iterator<Item=(String, &V)> + '_> {
        let res = self.0.entries().map(
            |(k, v): (&[char], &V)| (k.into_iter().collect(), v)
        );
        Box::new(res)
    }
    ...
    fn keys(&self) -> Box<dyn Iterator<Item=String> + '_> {
        let res = self.0.keys().map(|k: &[char]| k.into_iter().collect());
        Box::new(res)
    }
    ...
    /// The resulting Vec contains key-value-pairs sorted with the shortest keys first.
    fn search(&self, key: &str) -> Vec<(String, V)> {
        ...
        V: Clone + PartialEq
{
    ...
    fn keys(&self) -> Box<dyn Iterator<Item=&[K]> + '_> {
        let iter = if let Some(_) = &self.val {
    ...
    fn insert(&mut self, depth: usize, path: &[K], val: V) -> Option<V> {
        ...
        // and duplicate it because of rust
        let head = path[depth].clone();
        let head2 = head.clone();
        let head3 = head.clone();
        
        // if no next node, make a new one
        self.edges.entry(head2).or_insert_with(||
            Node::new_with_path(path[..=depth].to_owned())
        );
